    ...
    func getDataReady(){
        ...
        for( var index = 0; index != steps.count; index++){
            ...
            if(item.actionId == 1 || item.actionId == 20){//需要容器的
                let container = getOneContainer(item.ingridientId)
                segmentCells.insert([container.name,container.nameEng,"",container.largePhoto], atIndex: 0)                
            }else if(item.actionId == 3){//需要材料的
                let ingridient = getOneIngridient(item.ingridientId)
                NSLog("\(item.amount)")
                NSLog("\(item.unitId) " + UnitDictory[item.unitId]!)
                segmentCells.append([ingridient.name,ingridient.nameEng,"\(item.amount)"+UnitDictory[item.unitId]!,ingridient.largePhoto])
